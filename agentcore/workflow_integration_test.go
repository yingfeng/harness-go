...
// ============================================================
// P1-13: Server concurrent streaming runs + cancellation
// ============================================================

func TestWorkflow_Server_ConcurrentStreamCancel(t *testing.T) {
	...
	srv.RegisterGraph("echo-graph", cg)
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id int) {
			...
			if w.Code != http.StatusOK {
				errs <- fmt.Errorf("health %d: status %d", id, w.Code)
			}
		}(i)
	}
	wg.Wait()
