...
func (m *mockEvalModel) Generate(ctx context.Context, msgs []*schema.Message, opts ...agentcore.ModelOption) (*schema.Message, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *mockEvalModel) Stream(ctx context.Context, msgs []*schema.Message, opts ...agentcore.ModelOption) (*schema.StreamReader[*schema.Message], error) {
...
func newMockAgent(resp string) agentcore.Agent {
	m := &mockEvalModel{}
	m.addResp(resp)
...
func TestAgentError(t *testing.T) {
	...
	err = scorer(context.Background(), &EvalResult{Err: agentcore.ErrCancelTimeout})
	if err == nil {
...
func TestAgentErrorContains(t *testing.T) {
	...
	err := scorer(context.Background(), &EvalResult{
		Err: agentcore.ErrCancelTimeout,
	})
	if err != nil {
		t.Errorf("expected pass, got: %v", err)
	}
	...
	err = scorer(context.Background(), &EvalResult{Err: agentcore.ErrExecutionEnded})
	if err == nil {
