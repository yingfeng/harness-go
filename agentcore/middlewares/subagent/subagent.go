...
//	    Model:       parentModel,
//	    Middlewares: []agentcore.ReActMiddleware{mw, filesystemMW},
//	}
...
//   - Agent: a pre-built agentcore.Agent instance.
...
type SubAgentMiddleware struct {
	...
	cfg   *Config
	specs []SubAgentSpec
	tools []agentcore.Tool // AgentTool wrappers, built in ensureBuilt
...
func (m *SubAgentMiddleware) BindToConfig(config *agentcore.ReActConfig[*schema.Message]) {
	if m.built {
		return // idempotent
	}
...
func (m *SubAgentMiddleware) ensureBuilt(config *agentcore.ReActConfig[*schema.Message]) {
	if m.built {
		return
	}
	m.built = true
	...
	for _, spec := range m.specs {
		...
		var toolOpts []agentcore.AgentToolOption
		if m.cfg.EmitInternalEvents {
			toolOpts = append(toolOpts, agentcore.WithEmitInternalEvents())
		}
		if m.cfg.MaxDepth > 0 {
			toolOpts = append(toolOpts, agentcore.WithMaxDepth(m.cfg.MaxDepth))
		}
		tool := agentcore.NewAgentTool(context.Background(), agent, toolOpts...)
...
func (m *SubAgentMiddleware) resolveAgent(spec SubAgentSpec, parentCfg *agentcore.ReActConfig[*schema.Message]) agentcore.Agent {
	// 1. Build from AgentConfig (takes precedence).
	if spec.AgentConfig != nil {
		cfg := m.buildConfig(spec, parentCfg)
		return agentcore.NewReActAgent(cfg).
			WithName(spec.Name).
...
func (m *SubAgentMiddleware) buildConfig(spec SubAgentSpec, parentCfg *agentcore.ReActConfig[*schema.Message]) *agentcore.ReActConfig[*schema.Message] {
	cfg := spec.AgentConfig
	subCfg := &agentcore.ReActConfig[*schema.Message]{
		Model:         cfg.Model,
		Tools:         cfg.Tools,
...
func (m *SubAgentMiddleware) inheritedMiddlewares(parentCfg *agentcore.ReActConfig[*schema.Message], excludedNames []string) []agentcore.ReActMiddleware {
	excluded := make(map[string]bool, len(excludedNames)+1)
	for _, n := range excludedNames {
		excluded[n] = true
	}

	var inherited []agentcore.ReActMiddleware
...
func (m *SubAgentMiddleware) BeforeModelRewrite(ctx context.Context, state *agentcore.ReActAgentState, mc *agentcore.ModelContext) (context.Context, *agentcore.ReActAgentState, error) {
...
var _ agentcore.ReActMiddleware = (*SubAgentMiddleware)(nil)
