...
var planToolDef = agentcore.NewBaseTool(
	toolPlan,
	...
	func(ctx context.Context, args string) (string, error) {
		...
		if err := agentcore.SetRunLocalValue(ctx, sessionKeyPlan, plan); err != nil {
			return "", err
		}
		// Reset steps done when plan changes
		agentcore.SetRunLocalValue(ctx, sessionKeyStepsDone, 0)
		return fmt.Sprintf("Plan updated with %d steps", len(in.Steps)), nil
	},
)

// respondTool allows the replanner to signal completion.
var respondToolDef = agentcore.NewBaseTool(
	toolRespond,
...
func New(ctx context.Context, cfg *Config) (agentcore.ResumableAgent, error) {
	...
	replannerTools := make([]agentcore.Tool, 0, len(cfg.Replanner.Tools)+2)
	...
	loopAgent, err := agentcore.NewLoop(ctx, &agentcore.LoopConfig{
		Name:          agentNameLoop,
		Description:   "Plan-Execute-Replan loop",
		SubAgents:     []agentcore.Agent{executor, replanner},
	...
	seqAgent, err := agentcore.NewSequential(ctx, &agentcore.SequentialConfig{
		Name:        cfg.Name,
		Description: "Plan-Execute-Replan agent",
		SubAgents:   []agentcore.Agent{planner, loopAgent},
	})
	if err != nil {
...
func genPlannerInput(ctx context.Context, instruction string, input *agentcore.AgentInput) ([]*schema.Message, error) {
...
func genExecutorInput(ctx context.Context, instruction string, input *agentcore.AgentInput) ([]*schema.Message, error) {
...
func genReplannerInput(ctx context.Context, instruction string, input *agentcore.AgentInput) ([]*schema.Message, error) {
	planStr := getPlanStr(ctx)
	stepsDone := getStepsDone(ctx)
	objective := getObjective(input.Messages)
...
func getPlanStr(ctx context.Context) string {
	v, ok, err := agentcore.GetRunLocalValue(ctx, sessionKeyPlan)
	if err != nil || !ok || v == nil {
		return "(no plan yet)"
	}
	p, ok := v.(Plan)
...
func getStepsDone(ctx context.Context) int {
	v, ok, err := agentcore.GetRunLocalValue(ctx, sessionKeyStepsDone)
	if err != nil || !ok {
		return 0
	}
	if n, ok := v.(int); ok {
