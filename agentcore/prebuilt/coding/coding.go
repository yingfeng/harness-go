...
import (
	...
	"github.com/infiniflow/ragflow/harness/agentcore/profile"
...
func New(cfg *Config) *agentcore.ReActAgent[*schema.Message] {
	if cfg == nil {
		cfg = DefaultConfig()
	}
	...
	var middlewares []agentcore.ReActMiddleware
	...
	if len(cfg.SubAgentSpecs) > 0 {
		...
		reactCfg := &agentcore.ReActConfig[*schema.Message]{
			Model:         cfg.Model,
		...
		return agentcore.NewReActAgent(reactCfg)
	}
...
func (t *execTool) Invoke(ctx context.Context, args string, opts ...agentcore.ToolOption) (string, error) {
	return "", nil
}
func (t *execTool) Stream(ctx context.Context, args string, opts ...agentcore.ToolOption) (*schema.StreamReader[string], error) {
