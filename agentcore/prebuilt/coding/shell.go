...
func (m *ShellAllowListMiddleware) WrapToolInvoke(ctx context.Context, ep agentcore.InvokableToolEndpoint, tc *agentcore.ToolContext) (agentcore.InvokableToolEndpoint, error) {
...
func (m *ShellAllowListMiddleware) WrapEnhancedInvokableToolCall(ctx context.Context, ep agentcore.EnhancedInvokableToolEndpoint, tc *agentcore.ToolContext) (agentcore.EnhancedInvokableToolEndpoint, error) {
	if tc.Name != "execute" || m.cfg.Passthrough {
		return ep, nil
	}
	m.once.Do(m.init)
