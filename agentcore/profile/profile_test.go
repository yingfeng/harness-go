...
func (m *mockProfileModel) Generate(ctx context.Context, msgs []*schema.Message, opts ...agentcore.ModelOption) (*schema.Message, error) {
	return &schema.Message{Role: schema.RoleAssistant, Content: m.response}, nil
}
func (m *mockProfileModel) Stream(ctx context.Context, msgs []*schema.Message, opts ...agentcore.ModelOption) (*schema.StreamReader[*schema.Message], error) {
...
func TestRegisterAndParse(t *testing.T) {
	...
	RegisterProvider(&ProviderProfile{
		...
		InitModel: func(ctx context.Context, modelName string, opts map[string]any) (agentcore.Model[*schema.Message], error) {
...
func TestFilterMiddlewareByTypeName(t *testing.T) {
	...
	type mockMW struct{ agentcore.BaseMiddleware[*schema.Message] }
	type excludedMW struct{ agentcore.BaseMiddleware[*schema.Message] }

	mws := []agentcore.ReActMiddleware{
		&mockMW{},
		&excludedMW{},
		&mockMW{},
	}
...
func TestDualTrack(t *testing.T) {
	...
	agent, err := NewAgent(context.Background(), &AgentConfig{
		...
		Tools:              []agentcore.Tool{&simpleTool{name: "t1"}},
	})
	if err != nil {
		t.Fatalf("NewAgent error: %v", err)
	}
...
func (t *simpleTool) Invoke(ctx context.Context, args string, opts ...agentcore.ToolOption) (string, error) {
	return "result", nil
}
func (t *simpleTool) Stream(ctx context.Context, args string, opts ...agentcore.ToolOption) (*schema.StreamReader[string], error) {
