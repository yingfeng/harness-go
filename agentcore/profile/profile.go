...
type HarnessProfile struct {
	...
	ExtraMiddlewares []agentcore.ReActMiddleware
...
func NewAgent(ctx context.Context, cfg *AgentConfig) (agentcore.Agent, error) {
	if cfg == nil {
	...
	if harness := lookupHarness(cfg.HarnessProfileName); harness != nil && len(harness.ExcludedToolNames) > 0 {
		...
		filtered := make([]agentcore.Tool, 0, len(reactCfg.Tools))
	...
	return agentcore.NewReActAgent(reactCfg), nil
}

// buildModel resolves the model from ModelSpec + ProviderProfile.
func buildModel(ctx context.Context, cfg *AgentConfig) (agentcore.Model[*schema.Message], error) {
...
func buildReactConfig(ctx context.Context, cfg *AgentConfig) *agentcore.ReActConfig[*schema.Message] {
	// Start with system defaults.
	result := &agentcore.ReActConfig[*schema.Message]{
		MaxIterations: 10,
...
func filterMiddlewareByTypeName(mws []agentcore.ReActMiddleware, exclude []string) []agentcore.ReActMiddleware {
	if len(exclude) == 0 {
		return mws
	}
	excluded := makeMap(exclude)
	filtered := make([]agentcore.ReActMiddleware, 0, len(mws))
	for _, mw := range mws {
		if mw == nil {
			continue
		}
